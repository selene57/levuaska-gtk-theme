...
#
# Description:
#   An installation bash script for Levuaska GTK Theme, modified by Sam Hewitt's Paper GTK Theme installation script.
#
# Legal Stuff:
#
...
clear
echo '#-----------------------------------------#'
echo '#     Levuaska GTK Theme Install Script   #'
echo '#-----------------------------------------#'
...
if [ "$UID" -eq "$ROOT_UID" ]; then
	if [ -d /usr/share/themes/Levuaska ]; then
		echo
		show_question '\tFound an existing installation. Replace it? (Y)es, (N)o : ' 
		echo
		read INPUT
		case $INPUT in
			[Yy]* ) rm -Rf /usr/share/themes/Levuaska 2>/dev/null;;
			[Nn]* );;
		    * ) clear; show_error '\tSorry, try again.'; main;;
		esac
	fi
	echo "Installing..."
	cp -R ./Levuaska/ /usr/share/themes/
	chmod -R 755 /usr/share/themes/Levuaska
	echo "Installation complete!"
	echo "You will have to set your theme manually."
	end
elif [ "$UID" -ne "$ROOT_UID" ]; then
	if [ -d $HOME/.local/share/themes/Levuaska ]; then
		echo
		show_question '\tFound an existing installation. Replace it? (Y)es, (N)o : ' 
		echo
		read INPUT
		case $INPUT in
			[Yy]* ) rm -Rf "$HOME/.local/share/themes/Levuaska" 2>/dev/null;;
			[Nn]* );;
		    * ) clear; show_error '\tSorry, try again.'; main;;
		esac
	fi
	...
	cp -R ./Levuaska/ $HOME/.local/share/themes/
	# .themes
	install -d $HOME/.themes
	cp -R ./Levuaska/ $HOME/.themes/
	echo "Installation complete!"
	set
fi
}
...
function set {
echo
show_question '\tDo you want to set Levuaska as desktop theme? (Y)es, (N)o : ' 
echo
read INPUT
case $INPUT in
	[Yy]* ) settheme;;
...
function settheme {
echo "Setting Levuaska as desktop GTK theme..."
gsettings reset org.gnome.desktop.interface gtk-theme
gsettings reset org.gnome.desktop.wm.preferences theme
gsettings set org.gnome.desktop.interface gtk-theme "Levuaska"
gsettings set org.gnome.desktop.wm.preferences theme "Levuaska"
echo "Done."
setthemegnome
}
...
if [ -d /usr/share/gnome-shell/extensions/user-theme@gnome-shell-extensions.gcampax.github.com/ ]; then	
	echo
	show_question '\tWould you like to use Levuaska as your GNOME Shell theme? (Y)es, (N)o : '
	echo
	read INPUT
	case $INPUT in
		[Yy]* ) gsettings set org.gnome.shell.extensions.user-theme name "Levuaska";;
	    [Nn]* ) end;;
	    * ) echo; show_error "\aUh oh, invalid response. Please retry."; set;;
	esac
	end
...
if [ "$UID" -ne "$ROOT_UID" ]; then
	echo
	echo "Levuaska GTK Theme will be installed in:"
	echo
